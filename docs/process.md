...
| Метод разработки        | **Облегчённый Scrum**, код сразу в `main`                                                             |
...
| Реализация           | недели 1–9                                         | все по [team.md](team.md)             | коммиты в `main`                     |
| Проверка             | внутри этапа, не «в конце семестра»                | Шишков (QA), pytest парсера           | чек-лист, pytest                     |
...
Спринты могут накладываться по людям (Шишков уже верстает каталог и админку, пока Бондарь дописывает sync; Пягай пишет pytest cutoff, пока Бондарь делает job).
...
| Ретро                | по желанию, после этапов 2 и 4                | 15 мин: что мешало (синхрон, API)              |
...
Команда маленькая, PR не обязателен. Пишем и пушим коммиты в `main`.

```mermaid
flowchart LR
  Local[локальный_коммит]
  Main[main]
  Ci[CI_pytest]
  Local --> Main --> Ci
```
...
- Перед пушем — зелёный локальный запуск (или pytest по своей зоне).
- Feature-ветки и PR по желанию, если удобно изолировать длинную работу; в процесс это не входит.
- Шишков фиксирует скрин или чек-лист; Пягай — что изменилось в docs; Бондарь — pytest по своей зоне.
- Свой пункт этапа автор отмечает `[x]` в [sprints.md](sprints.md) в том же коммите, где задача сделана. Чужой пункт не закрывать.
- Релизы не нумеруются до защиты: поставка = зелёный `main` на момент демо этапа.

Definition of Done: миграция, если менялась схема; pytest / чек-лист QA / явный ручной шаг; OpenAPI и `docs/` совпадают с кодом (Пягай); `docker compose up` с нуля работает (Бондарь). Зоны кода — [team.md](team.md).

## Качество в цикле, а не в конце

- Парсер: регресс на HTML-фикстуре в том же коммите, который трогает селекторы.
...
CorpLunch сделан итеративно-инкрементально: пять наращиваемых поставок за ~9 недель. Метод — облегчённый Scrum без отдельного Scrum-мастера; код сразу в `main`, без обязательных PR. Требования в docs до кодирования, уточняются по инкрементам. Водопад не использовали: команда работает параллельно с первого дня.
