...
- **procurement** — сводка на дату, CSV/XLSX, отметка «заказ размещён», статистика.
...
В плане сотрудник видит **последнюю известную** цену из снимка каталога. В день заказа (после cutoff) система синхронизирует Mealty, пишет `actual_price` и помечает исчезнувшие блюда как `unavailable`. В сводку закупок входят только доступные позиции.
